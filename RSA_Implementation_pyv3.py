...
class RSA(object):
	...
	def __init__(self):
		...
		"""
		# Choose random primes between 1000000 and 5000000
	...
	def encrypt(self, msg):
		...
		hashed = ''.join([chr(h) for h in hashedVals])
	...
	def decrypt(self, hashedMsg):
		...
		hashedVals = [ord(c) for c in hashedMsg]
		msgVals = [(h ** self.d) % self.n for h in hashedVals]
		msg = ''.join([chr(m) for m in msgVals])
